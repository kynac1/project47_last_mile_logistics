...
def sample_generator(rg:np.random.Generator, n):
    ...
    lat, lon = get_sample(n, rg, cd, sample_df, CHC_df, CHC_sub, CHC_sub_dict, save=False)
    ...
    customers = [Customer(lat[i],lon[i], 0.9, 0.9, presence_list[i], rg) for i in range(len(lat))]
