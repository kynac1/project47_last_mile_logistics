...
def test_reroute_sim_tw():
    ...
    windows = np.array(
        [[0.0, 10000.0], [0.0, 10000.0], [0.0, 10000.0], [2.0, 3.0], [0.0, 10000.0]]
    )
    ...
    distance, time, futile, delivered = sim1(
        s, update_function10(distances, times, windows, customers)
    )
...
def update_function20(distance_matrix, time_matrix, time_windows, customers):
    ...
    def h(route, i, time):
        ...
        next_time = g(route[i], route[i + 1], time)
        if (
            route[i + 1] in time_windows
        ):  # not sure if this line is needed as all loc has a tw?
            # out of time window
            if (
