...
def route_optimizer(depots, dm, tm, time_windows, day, arrival_days, futile_count, alternate_locations):
    locs = dm.shape[0]
...
def test_alternate_locations():
    ...
    def sample_generator(rg:np.random.Generator):
        lat, lon = get_sample(20, rg, cd, sample_df, CHC_df, CHC_sub, CHC_sub_dict, save=False)
        time_windows = np.zeros((len(lat),2))
        for i in range(len(lat)):
            time_windows[i,0] = 0
            time_windows[i,1] = 28800
